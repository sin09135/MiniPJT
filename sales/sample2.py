...
df2 = pd.read_csv('data/비골목_streamlit.csv')
...
if selected_feature1 in df1['상권_코드_명'].unique().tolist():
    selected_dataframe = df1  # df1에서 선택한 값을 찾았으므로 해당 데이터프레임을 선택합니다.
else:
    selected_dataframe = df2  # df2에서 선택한 값을 찾았거나 둘 다에 없을 경우 df2를 선택합니다.

# 선택한 값에 해당하는 피처를 불러옵니다.
selected_feature_data = selected_dataframe[selected_dataframe['상권_코드_명'] == selected_feature1]

# 이제 selected_feature_data를 사용하여 모델에 피처로 넣을 수 있습니다.
...
user_input = []
for i, feature_name in enumerate(selected_feature_data.columns):
    max_value_feature = float(df1[feature_name].max())
    min_value_feature = float(df1[feature_name].min())

    if i == 14:
        # 15번째 피처에 대한 처리
        user_input.append(st.text_input(f"({feature_name}):"))
    else:
